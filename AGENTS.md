...
For any AI assistant working in this repository: Claude, Kimi, Cursor, Codex,
Gemini CLI, or a person reading over one's shoulder. `CLAUDE.md` points here so
there is one copy to keep current rather than two that drift.
...
Nothing important is on one laptop. If you are picking this up on a new
machine, this is the whole inventory.

| What | Where | How to get in |
| --- | --- | --- |
| The code | GitHub, `joansonltd-beep/expeditions` | Your GitHub account |
| The live site | Vercel, deploys on every push to `main` | Vercel account, or just push |
| Editable content | Sanity | sanity.io/manage, or `/studio` on the live site |
| Photographs and video | `public/` in this repo | The repo |
| The print brochure | `design/brochure/` in this repo | The repo. See below |
| Secrets | `.env.local`, gitignored, never committed | Sanity dashboard, regenerate if lost |

**Nothing lives only on a laptop except `.env.local`.** Losing a machine costs
you the tokens, and tokens are reissued in a minute from sanity.io/manage.

### Editing content without touching code

Most copy is editable at **/studio on the live site**, from any browser, with a
Sanity login. Services, prices, the About page, the hero, contact details. That
route needs no checkout, no Node and no terminal.

Code changes need the repo. A browser-based editor (GitHub Codespaces, or any
hosted agent with repo access) works as well as a laptop.

### The print brochure

`design/brochure/` holds the source, not just the PDF:

- `Main.dc.html` is the outside face, `Inside.dc.html` is the inside face. Each
  is one artboard, laid out for Letter landscape at 96px per inch: 1056 x 816,
  three 352px panels per sheet.
- `build-print.mjs` inlines the images as data URIs, strips the dashed fold
  guides, and writes `sheet1.html` and `sheet2.html`.
- To produce the PDF: run the script, screenshot each sheet with headless
  Chrome at `--force-device-scale-factor=3.125` (that is 300dpi, 3300 x 2550),
  then place the two images full-page and print to PDF.
- It is flattened to one image per page on purpose. PDF viewers antialias the
  edge of every embedded image, which showed up as hairlines tracing each
  photograph. One image per page leaves no internal edges for that to happen at.
