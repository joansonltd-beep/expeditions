...
/**
 * Floating contact controls, bottom-right on every page.
 *
 * Restyled from two identical circles to a WhatsApp pill that names itself and
 * a smaller, quieter chat button. They were previously the same size and the
 * same colour, distinguishable only by icon, which was a real usability
 * problem once the seasonal skin turned both of them red.
 *
 * Smaller than before, with a hairline ring instead of a heavy glow, and the
 * label collapses on the narrowest screens so the pair never crowds the
 * content behind it.
 */
    ...
    <div className="fixed bottom-4 right-4 z-40 flex flex-col items-end gap-2.5 sm:bottom-6 sm:right-6">
      {chatbotUrl ? (
        <a
          ...
          className="grid h-10 w-10 place-items-center rounded-full bg-cream text-navy shadow-sm ring-1 ring-navy/15 transition hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2"
        >
      ...
      <a
        href={waLink("Hi Jo, I have a question about moving within CARICOM.")}
        target="_blank"
        rel="noopener noreferrer"
        onClick={() => track("whatsapp_click", { location: "float" })}
        className="inline-flex items-center gap-2 rounded-full bg-whatsapp py-2.5 pl-3 pr-3 text-sm font-semibold text-white shadow-md ring-1 ring-black/10 transition hover:brightness-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-whatsapp focus-visible:ring-offset-2 sm:pr-4"
      >
        ...
        <span className="hidden sm:inline">WhatsApp Jo</span>
      </a>
    </div>
  );
}
